...
class BookingService:
    ...
    def create_booking(self, room_id, customer_id, check_in_date, check_out_date, 
        ...
        booking = Booking(
            room_id=room_id, customer_id=customer_id, check_in_date=check_in_date,
            check_out_date=check_out_date, status=status,
            early_hours=early_hours, late_hours=late_hours
        )
        try:
    ...
    def cancel_booking(self, booking_id):
        ...
        room = booking.room 
        if not room: room = self.db_session.get(Room, booking.room_id)
        ...
        if room:
            ...
            room.status = Room.STATUS_BOOKED if other_active_bookings > 0 else Room.STATUS_AVAILABLE
